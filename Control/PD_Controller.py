...
topicname_command = '/maccepa/motors/set_position'
...
topicname_maccepa_feedback = '/maccepa/motors/states'

topicname_joint_states = '/maccepa/gazebo_joint_states'
# topicname_joint_states = '/maccepa/joint_states'
...
# MACCEPA
MACCEPA_res = {}
with open (os.path.join(os.path.dirname(__file__), 'MACCEPA_res0'), 'rb') as fp:
    MACCEPA_res[0] = pickle.load(fp)
with open (os.path.join(os.path.dirname(__file__), 'MACCEPA_res1'), 'rb') as fp:
    MACCEPA_res[1] = pickle.load(fp)
...
def get_index(phi1, phi2, motor_index):
    ...
    index1 = (phi1 - MACCEPA_res[motor_index]['min_deflection_angle']) / MACCEPA_res[motor_index]['step_deflection_angle']
    index2 = (phi2 - MACCEPA_res[motor_index]['min_motor2_angle'])     / MACCEPA_res[motor_index]['step_motor2_angle']

    index1 = int(index1)
    index2 = int(index2)

    if index1 > MACCEPA_res[motor_index]['Count']:
        raise NameError('requested unsupported value of angle phi1')
    if index2 > MACCEPA_res[motor_index]['Count']:
        raise NameError('requested unsupported value of angle phi2')

    return index1, index2


def get_linear_model(phi1, phi2, motor_index):

    index1, index2 = get_index(phi1, phi2, motor_index)
    
    phi1_table = MACCEPA_res[motor_index]['deflection_angle'][index1,   index2]
    phi2_table = MACCEPA_res[motor_index]['motor2_angle']    [index1,   index2]
    ...
    if index1 < (1.0 * MACCEPA_res[motor_index]['Count'] / 2):
        delta1 = 1
    else:
        delta1 = -1
    
    if index2 < (1.0 * MACCEPA_res[motor_index]['Count'] / 2):
        delta2 = 1
    else:
        delta2 = -1
    
    tau0  = MACCEPA_res[motor_index]['torque'][index1,        index2]
    tau11 = MACCEPA_res[motor_index]['torque'][index1+delta1, index2]
    tau12 = MACCEPA_res[motor_index]['torque'][index1,        index2+delta2]
    
    tau11_long = MACCEPA_res[motor_index]['torque'][index1+delta1*long_linearization_constant, index2]
    tau12_long = MACCEPA_res[motor_index]['torque'][index1,                                    index2+delta2*long_linearization_constant]
    
    T      = np.array([(tau11      - tau0) / (delta1*MACCEPA_res[motor_index]['step_deflection_angle']),
                       (tau12      - tau0) / (delta2*MACCEPA_res[motor_index]['step_motor2_angle'])  ])
    
    T_long = np.array([(tau11_long - tau0) / (delta1*long_linearization_constant*MACCEPA_res[motor_index]['step_deflection_angle']),
                       (tau12_long - tau0) / (delta2*long_linearization_constant*MACCEPA_res[motor_index]['step_motor2_angle'])])
    
    delta_tau = T.dot(np.array([delta_phi1, delta_phi2]))
    tau0 = tau0 + delta_tau

    k0  = MACCEPA_res[motor_index]['stiffness'][index1,        index2]
    k11 = MACCEPA_res[motor_index]['stiffness'][index1+delta1, index2]
    k12 = MACCEPA_res[motor_index]['stiffness'][index1,        index2+delta2]
    
    k11_long = MACCEPA_res[motor_index]['stiffness'][index1+delta1*long_linearization_constant, index2]
    k12_long = MACCEPA_res[motor_index]['stiffness'][index1,                                    index2+delta2*long_linearization_constant]

    K      = np.array([(k11      - k0) / (delta1*MACCEPA_res[motor_index]['step_deflection_angle']),
                       (k12      - k0) / (delta2*MACCEPA_res[motor_index]['step_motor2_angle'])      ])

    K_long = np.array([(k11_long - k0) / (delta1*long_linearization_constant*MACCEPA_res[motor_index]['step_deflection_angle']),
                       (k12_long - k0) / (delta2*long_linearization_constant*MACCEPA_res[motor_index]['step_motor2_angle'])     ])
...
def gravity_compensation(q1, q2):
    ...
    G = -np.array([(L2*sin(pi + q1 + q2)*(m3 + m2/2)*g + L1*sin(pi + q1)*(m3 + m2 + m1/2)*g),
                   (L2*sin(pi + q1 + q2)*(m3 + m2/2)*g)])
...
def pizza_teleportation(time_abs):
    ...
    if (time_pizza_appears[0] < t) and (t < time_pizza_appears[1]):
        pizza_position = np.array([0, 0, 1.5])
        ToTeleport = True
    ...
    if ToTeleport:
        ...
        state.model_name = 'brick1'
        state.pose.position.x = pizza_position[0]+0.00+0.05
        state.pose.position.y = pizza_position[1]-0.03
        state.pose.position.z = pizza_position[2]+0.05
        publisher_pizza_id[0].publish(state) 
        
        state.model_name = 'brick2'
        state.pose.position.x = pizza_position[0]+0.05+0.05
        state.pose.position.y = pizza_position[1]+0.03
        state.pose.position.z = pizza_position[2]+0.1
        publisher_pizza_id[0].publish(state) 
        
        state.model_name = 'brick3'
        state.pose.position.x = pizza_position[0]-0.05+0.05
        state.pose.position.y = pizza_position[1]+0.03
        state.pose.position.z = pizza_position[2]+0.15
        publisher_pizza_id[0].publish(state) 
...
def publish_phi(phi11, phi12, phi21, phi22):
    ...
    out = MotorsPose(phi11, phi12, phi21, phi22)
    publisher_ids[0].publish(out)
...
def callback_JointState(JointState):
    ...
    q[0] = current_joint_position[names.index("shoulder")]
    q[1] = current_joint_position[names.index("elbow")]

    v = current_joint_velocity[0:2]
    v[0] = current_joint_velocity[names.index("shoulder")]
    v[1] = current_joint_velocity[names.index("elbow")]
    ##################
    ...
    tau01, k01, T1, T1_long, K1, K1_long = get_linear_model(phi11, phi12, 0)
    tau02, k02, T2, T2_long, K2, K2_long = get_linear_model(phi21, phi22, 1)
    ##################
    ...
    if Verbose:
        ...
        print "tau model: ", np.array([tau01, tau02])
...
def callback_encoders(msg):
    ...
    current_maccepa_shoulder_position = np.array([msg.motor_1_pose, msg.motor_2_pose])
    current_maccepa_elbow_position = np.array([msg.motor_3_pose, msg.motor_4_pose])
...
def listener_JointState():
    ...
    # create publisher
    publisher_ids[0] = rospy.Publisher(topicname_command, MotorsPose, queue_size=10)
    ...
    rospy.Subscriber(topicname_maccepa_feedback, MotorsPose, callback_encoders, queue_size=1)
    print "Created Subscriber: ", topicname_maccepa_feedback
