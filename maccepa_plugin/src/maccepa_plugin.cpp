...
double Ad[N][N] = {{1.0, 0.0009516},
                   {0,   0.9048}
};
double Bd[N][U] = {{0.0004837,   -0.0004837},
                   {0.9516, -0.9516}
};
...
void maccepaPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    if(_sdf->HasElement("mode")) {
        this->mode = static_cast<Mode>(_sdf->GetElement("mode")->Get<int>());
    }
    ...
    if(_sdf->HasElement("motors_angle_limit")) {
        motors_angle_limit = _sdf->GetElement("motors_angle_limit")->Get<double>();
    }
    if(_sdf->HasElement("motors_velocity_limit")) {
        motors_velocity_limit = _sdf->GetElement("motors_velocity_limit")->Get<double>();
    }
    if(_sdf->HasElement("motors_torque_limit")) {
        motors_torque_limit = _sdf->GetElement("motors_torque_limit")->Get<double>();
    }
...
void maccepaPlugin::cardioid_motor_callback(const std_msgs::Float64::ConstPtr &msg) {
    ...
    } else if(this->mode == MODE_SPEED) {
        this->q1_dot_ref = msg->data;
    }
...
void maccepaPlugin::motor_ref_limit(double &q1_ref_next, double &q2_ref_next) {
    ...
    if(this->motors_angle_limit > 0) {
        /* check if q1_ref_next is over saturation limit*/
        if (q1_ref_next < -this->motors_angle_limit) {
            q1_ref_next = -this->motors_angle_limit;
        }
        if (q1_ref_next > this->motors_angle_limit) {
            q1_ref_next = this->motors_angle_limit;
        }
...
void maccepaPlugin::motor_torque_limit(double &u) {
    ...
    if(this->motors_torque_limit > 0) {
        /* check if u value is over saturation limit*/
        if (u < -this->motors_torque_limit) {
            u = -this->motors_torque_limit;
        }
...
void maccepaPlugin::system_update(double *x, double *y, double *u) {
    ...
    /* speed saturation */
    if(this->motors_velocity_limit > 0) {
        if (x_next[1] > this->motors_velocity_limit) {
            x_next[1] = this->motors_velocity_limit;
        } else if (x_next[1] < -this->motors_velocity_limit) {
            x_next[1] = -this->motors_velocity_limit;
        }
        /* position saturation due to speed saturation */
        if (x_next[0] - x[0] > this->motors_velocity_limit * Ts) {
            x_next[0] = x[0] + this->motors_velocity_limit * Ts;
            x_next[1] = this->motors_velocity_limit;
        } else if (x_next[0] - x[0] < -this->motors_velocity_limit * Ts) {
            x_next[0] = x[0] - this->motors_velocity_limit * Ts;
            x_next[1] = -this->motors_velocity_limit;
        }
    }
...
void maccepaPlugin::updateMotorState(double &e, double &q, double &q_dot, double *x, double *y, double *u,
...
void maccepaPlugin::OnUpdate(const common::UpdateInfo & /*_info*/) {
    ...
    if(this->mode == MODE_POSITION) {
        ...
        updateMotorState(this->e1_prev, this->q1, this->q1_dot,
                         x1_k, y1_k, u1_k,
                         this->q1_ref_prev, this->q1_prev, this->e1_prev, this->torque1_external);
        /* update pretension motor state */
        updateMotorState(this->e2_prev, this->q2, this->q2_dot,
                         x2_k, y2_k, u2_k,
                         this->q2_ref_prev, this->q2_prev, this->e2_prev, this->torque2_external);
    } else if(this->mode == MODE_SPEED) {
        /* update cardioid motor state */
        updateMotorState(this->e1_prev, this->q1, this->q1_dot,
                         x1_k, y1_k, u1_k,
                         this->q1_dot_ref_prev, this->q1_dot_prev, this->e1_prev, this->torque1_external);
        /* update pretension motor state */
        updateMotorState(this->e2_prev, this->q2, this->q2_dot,
                         x2_k, y2_k, u2_k,
                         this->q2_dot_ref_prev, this->q2_dot_prev, this->e2_prev, this->torque2_external);
    }
    ...
    msg_test.data[1] = this->torque1_external;
    msg_test.data[2] = this->torque2_external;
    ...
    this->q1_dot_prev = this->q1_dot;
    this->q1_ref_prev = this->q1_ref;
    this->q1_dot_ref_prev = this->q1_dot_ref;

    this->q2_prev = this->q2;
    this->q2_dot_prev = this->q2_dot;
    this->q2_ref_prev = this->q2_ref;
    this->q2_dot_ref_prev = this->q2_dot_ref;
